...
const promiseMap = new Map<
  string,
  DeferredPromise<JSONSerializable | undefined>
>();
...
function isIframeContextChange(message: any): message is IframeContextChange {
  return (
    typeof message == "object" && message?.type === "iframe-context-change"
  );
}

const isAdapterResponseMessage = (
  message: any
): message is AdapterResponseMessage => {
  return (
...
export function makeWakuObjectAdapter(): WakuObjectAdapter {
  ...
  async function sendTransaction(to: string, token: Token) {
    const response = await adapterFunction("sendTransaction")(
      to,
  ...
  async function rpcRequest(method: string, params: any) {
    const response = await adapterFunction("rpcRequest")(
      method,
...
export function makeWakuObjectContext(
  ...
  contextProps?: Partial<WakuObjectContextProps>
): WakuObjectContext {
  ...
  async function updateStore(
    updater: (store?: JSONSerializable) => JSONSerializable
  ) {
